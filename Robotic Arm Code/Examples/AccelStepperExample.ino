...
AccelStepper stepper(1, 5, 22);    // direction Digital 9 (CCW), pulses Digital 8 (CLK)
...
void setup()
{
    ...
    stepper.setAcceleration(20000); // ACCELERATION = Steps /(second)^2
...
void RotateAbsolute()
{
    ...
    runallowed = true;                                   // allow running - this allows entering the RunTheMotor() function.
    stepper.setMaxSpeed(receivedSpeed);                  // set speed
    stepper.moveTo(directionMultiplier * receivedSteps); // set absolute distance
}
